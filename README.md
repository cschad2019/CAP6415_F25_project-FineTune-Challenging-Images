...
## Reproducibility
- Seeds set in code (`seed` in config/CLI) and `torch.use_deterministic_algorithms(True)` enabled; expect deterministic kernels where supported. Document any ops that complain and flip to `False` if needed.
- No external data beyond CIFAR-10; paths use repo-relative defaults.
- Checkpoints saved from evaluated weights (EMA-aware) so reported best metrics match the saved model.
